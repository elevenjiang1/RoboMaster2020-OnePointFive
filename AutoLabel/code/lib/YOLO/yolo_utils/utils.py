...
def one_class_non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.4):
    """
    Removes detections with lower object confidence score than 'conf_thres' and performs
    Non-Maximum Suppression to further filter detections.
    Returns detections with shape:
    ...
    output = [None for _ in range(len(prediction))]#此处生成了len(prediction)个None
    for image_i, image_pred in enumerate(prediction):#对每一个predection进行操作
        # Filter out confidence scores below threshold
